...
(function (window, angular) {
    ...
    angular.module('mwl.calendar').run([
        ...
        function ($templateCache) {
            ...
            $templateCache.put('src/templates/calendarMonthView.html', '<div class="cal-row-fluid cal-row-head"><div class="cal-cell1" ng-repeat="day in vm.weekDays track by $index">{{ day }}</div><hr class="calendar-grid-heading-stripe"></div><div class="cal-month-box"><div ng-repeat="rowOffset in vm.monthOffsets track by rowOffset"><div class="cal-row-fluid cal-before-eventlist"><div ng-repeat="day in vm.view | calendarLimitTo:7:rowOffset track by $index" class="cal-cell1 cal-cell {{ day.highlightClass }}" ng-click="vm.dayClicked(day)" ng-class="{pointer: day.events.length > 0}"><div ng-include="\'src/templates/calendarMonthDay.html\'"></div></div></div></div></div><mwl-calendar-slide-box class="cal-month-box-event-list" is-open="true" events="vm.openEvents" unscheduled-events="unscheduledEvents" on-event-click="onEventClick" edit-event-html="editEventHtml" on-edit-event-click="onEditEventClick" delete-event-html="deleteEventHtml" on-delete-event-click="onDeleteEventClick"></mwl-calendar-slide-box>');
            $templateCache.put('src/templates/calendarSlideBox-original.html', '');
            $templateCache.put('src/templates/calendarSlideBox.html', '<div class="cal-slide-box" collapse="vm.shouldCollapse" mwl-collapse-fallback="vm.shouldCollapse"><div class="cal-slide-content cal-event-list"><ul class="unstyled list-unstyled"><div ng-show="events.length === 0" class="event-list-header"><h4 class="gray-light"><em>No events scheduled for day</em></h4></div><div class="row"><div class="col-xs-12" ng-repeat="event in events track by $index | orderBy:\'event.partParameters.startTime\':true"><div class="row"><div class="col-xs-2"><span class="scheduled-tasks-date ts-burgundy">{{calendarDate}} <span ng-if="$first">TODAY</span></span><br><span class="scheduled-tasks-date ts-burgundy">{{event.partParameters.startTime | date: \'EEEE\'}}</span><br><span class="scheduled-tasks-date ts-burgundy">{{event.partParameters.startTime | date: \'MMM. dd\' }}</span></div><div class="col-xs-2"><span class="scheduled-tasks-time gray-light">{{event.partParameters.startTime | date: \'h:mm a\'}}</span></div><div class="col-xs-6 event-list-primary-details"><span class="small icon-{{ icons[event.assessmentPartType] }} gray-dark"><span class="event-list-title gray-dark"><a href="javascript:;" class="event-item gray-dark" ng-click="onEventClick({calendarEvent: event})">{{event.title }}</a></span><br><span class="event-list-event-type pull-left gray-dark"><span class="tree-left-icon small icon-{{ icons[event.assessmentPartType] }} pull-left"></span> {{event.assessmentPartType | lowercase}}</span><br><span class="event-list-observer pull-left gray-dark">Observer: {{event.actorLastNameFirstName}}</span></span></div><div class="col-xs-2"><span ng-show="event.actorId === $scope.currentUserActorId" class="pull-right event-list-action ts-blue"><button class="btn btn-primary" ng-click="logDay()">reschedule</button></span> <a href="javascript:;" class="pull-right ts-blue event-item-edit" ng-if="editEventHtml && event.editable !== false" ng-bind-html="vm.$sce.trustAsHtml(editEventHtml)" ng-click="onEditEventClick({calendarEvent: event})"></a></div></div><hr class="event-list-separator" ng-if="!$last"></div></div></ul></div></div>');
